...
// In NeoForge 21.0/21.1, ModConfigEvent subtypes (IModBusEvent) can only be dispatched
// on the mod event bus — `@EventBusSubscriber` without `bus = MOD` targets the common
// game bus and triggers "IModBusEvent events are not allowed on the common NeoForge bus".
// Mod-bus targeting is fine on newer versions too; keep it explicit everywhere.
