...
public class AutoHideHUD {
    ...
    // Accessor so the mixin (item-render tinting) can read the current fade alpha.
    ...
    public void onClientTick(ClientTickEvent.Post event) {
        ...
        if (player == null) {
            ...
            TEMP_PLAYER_DATA.selectedHotbarSlot = inventory.selected;
            TEMP_PLAYER_DATA.vehicleHealth = .01f;
            TEMP_PLAYER_DATA.maxVehicleHealth = .01f;
            // 1.21.4's Screen doesn't have isInGameUi / isPauseScreen helpers — treat any non-sleep screen as "in menu".
...
//            TEMP_PLAYER_DATA.showYawPitch = AutoHideHUDConfig.showYawPitch.get();
    ...
    public void onRenderHUDPre(RenderGuiLayerEvent.Pre event) {
        ...
        if (!preventHide && currentTick - lastUpdatedTick > tickDuration) {
            ...
            if (shouldHideLayer(event.getName())) {
                if (currentAlpha <= targetAlpha) {
                    // Fade is already complete — clamp and fall through so the shader-color
                    // push at the bottom still fires. Previously we returned early here,
                    // which on 1.21/1.21.1 caused the HUD to pop back to full opacity as
                    // soon as the fade finished (setShaderColor never got set for that frame).
                    alpha = currentAlpha;
        ...
        // MC 1.21/1.21.1 GuiGraphics.blit / blitSprite don't accept a color arg (the
        // Function+color overloads arrive in 1.21.2), so our mixin can't fade hearts,
        // hotbar frame, food, armor, XP bar, etc. via @ModifyVariable. Push the fade
        // through the GUI shader's global color so every draw in this layer multiplies
        // by the current alpha; Post resets it back to 1. Fills/text still get the
        // mixin path on top, which compounds slightly (final alpha ≈ alpha²) but the
        // visual result is "everything fades together" instead of "only text fades".
        if (alpha < 1f && shouldHideLayer(event.getName())) {
            // Crosshair uses GL_ONE_MINUS_DST_COLOR / GL_ONE_MINUS_SRC_COLOR (inverting)
    ...
    public void onRenderHUDPost(RenderGuiLayerEvent.Post event) {
        // Restore default shader color so world rendering and other mods' overlays
        // aren't tinted. Safe to call unconditionally — idempotent when already 1.
    ...
    private boolean shouldHideLayer(ResourceLocation layerName) {
        ...
        if (AutoHideHUDConfig.hideExperienceLevel.get() && layerName.equals(VanillaGuiLayers.EXPERIENCE_LEVEL))
            return true;
        // In 1.21.4 the XP progress bar and jump meter are separate layers (they were merged into
        // VanillaGuiLayers.CONTEXTUAL_INFO_BAR in 1.21.5+). Tie them to the existing experience toggle.
        if (AutoHideHUDConfig.hideExperienceLevel.get() && layerName.equals(VanillaGuiLayers.EXPERIENCE_BAR))
            return true;
        if (AutoHideHUDConfig.hideExperienceLevel.get() && layerName.equals(VanillaGuiLayers.JUMP_METER))
            return true;
